...
<cfset patientService  = createObject("component","MedicalManagementSystem.components.PatientService")>
                    ...
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="blood_group" id="add_blood_group" class="form-select">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="emergency_name" id="add_emergency_name"
                                       class="form-control">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="emergency_phone" id="add_emergency_phone"
                                       class="form-control">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="blood_group" id="editBloodGroup" class="form-select">
                                    <option value="">Select Blood Group</option>
                                    <option value="A+">A+</option>
                                    <option value="A+">A-</option>
                                    <option value="A+">B+</option>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="emergency_name" id="editEmergencyName"
                                       class="form-control">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="emergency_phone" id="editEmergencyPhone"
                                       class="form-control">
...
$(document).ready(function () {
    ...
    var rules = {
        ...
        // bloodGroup: function (v) {
        //     return $.trim(v).length === 0 || /^(A|B|AB|O)[+-]$/i.test($.trim(v));
        // },
    ...
    function validateAddForm() {
        ...
        if (!rules.required(fullName)) {
            setError('add_full_name','add_full_name_err','Full Name is required.'); ok = false;
        } else if (!rules.minLen(fullName, 3)) {
            setError('add_full_name','add_full_name_err','Full Name must be at least 3 characters.'); ok = false;
        } else if (!rules.alpha(fullName)) {
            setError('add_full_name','add_full_name_err','Full Name must contain letters only.'); ok = false;
        } else { setValid('add_full_name'); }

        var username = $('#add_username').val();
        if (!rules.required(username)) {
            setError('add_username','add_username_err','Username is required.'); ok = false;
        } else if (!rules.minLen(username, 3)) {
            setError('add_username','add_username_err','Username must be at least 3 characters.'); ok = false;
        } else if (!rules.alphaNum(username)) {
            setError('add_username','add_username_err','Letters, numbers and underscores only.'); ok = false;
        } else { setValid('add_username'); }

        var email = $('#add_email').val();
        if (!rules.required(email)) {
            setError('add_email','add_email_err','Email is required.'); ok = false;
        } else if (!rules.email(email)) {
            setError('add_email','add_email_err','Please enter a valid email address.'); ok = false;
        } else { setValid('add_email'); }

        var phone = $('#add_phone').val();
        if (!rules.required(phone)) {
            setError('add_phone','add_phone_err','Phone is required.'); ok = false;
        } else if (!rules.phone(phone)) {
            setError('add_phone','add_phone_err','Enter valid phone number.'); ok = false;
        } else { setValid('add_phone'); }

        // var bg = $('#add_blood_group').val();
        // if (rules.required(bg) && !rules.bloodGroup(bg)) {
        //     setError('add_blood_group','add_blood_group_err','Enter a valid blood group (e.g. A+, O-, AB+).'); ok = false;
        // } else if (rules.required(bg)) { setValid('add_blood_group'); }
        ...
        if (!rules.required(eName)) {
            setError('add_emergency_name','add_emergency_name_err','Emergency Contact Name is required.'); ok = false;
        } else if (!rules.minLen(eName, 3)) {
            setError('add_emergency_name','add_emergency_name_err','Must be at least 3 characters.'); ok = false;
        } else { setValid('add_emergency_name'); }

        var ePhone = $('#add_emergency_phone').val();
        if (!rules.required(ePhone)) {
            setError('add_emergency_phone','add_emergency_phone_err','Emergency Contact Phone is required.'); ok = false;
        } else if (!rules.phone(ePhone)) {
            setError('add_emergency_phone','add_emergency_phone_err','Must be 10-15 digits.'); ok = false;
        } else { setValid('add_emergency_phone'); }

        var dob = $('#add_dob').val();
            if (!rules.required(dob)) {
                setError('add_dob','add_dob_err','Date of Birth is required.'); ok = false;
            } else if (!rules.dob(dob)) {
                setError('add_dob','add_dob_err','Date of Birth cannot be in the future.'); ok = false;
    ...
    function validateEditForm() {
        ...
        if (!rules.required(fullName)) {
            setError('editFullName','edit_full_name_err','Full Name is required.'); ok = false;
        } else if (!rules.minLen(fullName, 3)) {
            setError('editFullName','edit_full_name_err','Must be at least 3 characters.'); ok = false;
        } else if (!rules.alpha(fullName)) {
            setError('editFullName','edit_full_name_err','Letters only.'); ok = false;
        } else { setValid('editFullName'); }

        var email = $('#editEmail').val();
        if (!rules.required(email)) {
            setError('editEmail','edit_email_err','Email is required.'); ok = false;
        } else if (!rules.email(email)) {
            setError('editEmail','edit_email_err','Enter a valid email address.'); ok = false;
        } else { setValid('editEmail'); }

        var phone = $('#editPhone').val();
        if (!rules.required(phone)) {
            setError('editPhone','edit_phone_err','Phone is required.'); ok = false;
        } else if (!rules.phone(phone)) {
            setError('editPhone','edit_phone_err','Must be 10–15 digits.'); ok = false;
        } else { setValid('editPhone'); }

        // var bg = $('#editBloodGroup').val();
        // if (rules.required(bg) && !rules.bloodGroup(bg)) {
        //     setError('editBloodGroup','edit_blood_group_err','Enter a valid blood group (e.g. A+, O-, AB+).'); ok = false;
        // } else if (rules.required(bg)) { setValid('editBloodGroup'); }
        ...
        if (!rules.required(eName)) {
            setError('editEmergencyName','edit_emergency_name_err','Emergency Contact Name is required.'); ok = false;
        } else if (!rules.minLen(eName, 3)) {
            setError('editEmergencyName','edit_emergency_name_err','Must be at least 3 characters.'); ok = false;
        } else { setValid('editEmergencyName'); }

        var ePhone = $('#editEmergencyPhone').val();
        if (!rules.required(ePhone)) {
            setError('editEmergencyPhone','edit_emergency_phone_err','Emergency Contact Phone is required.'); ok = false;
        } else if (!rules.phone(ePhone)) {
            setError('editEmergencyPhone','edit_emergency_phone_err','Must be 10–15 digits.'); ok = false;
        } else { setValid('editEmergencyPhone'); }

        var dob = $('#editDOB').val();
        if (!rules.required(dob)) {
            setError('editDOB','edit_dob_err','Date of Birth is required.'); ok = false;
        } else if (!rules.dob(dob)) {
            setError('editDOB','edit_dob_err','Date of Birth cannot be in the future.'); ok = false;
    ...
    $('#addPatientForm').on('submit', function (e) {
        ...
        $.ajax({
            url:      '/MedicalManagementSystem/components/PatientService.cfc'
                    + '?method=addPatientAjax&returnformat=json',
            type:     'POST',
            data:     $(this).serialize()
    ...
    $('#editPatientForm').on('submit', function (e) {
        ...
        $.ajax({
            url:      '/MedicalManagementSystem/components/PatientService.cfc'
                    + '?method=editPatientAjax&returnformat=json',
            type:     'POST',
            data:     $(this).serialize(),
            dataType: 'json',
            success: function (res) {
                ...
                var $editBtn = $('#patientsTable')
                    .find('.editBtn[data-patient_id="' + pid + '"]');
