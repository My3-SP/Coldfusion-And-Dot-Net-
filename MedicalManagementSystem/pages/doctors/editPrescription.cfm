    ...
    <div class="card p-4 shadow-sm">
        ...
        <div class="mb-3">
            ...
            <textarea id="diagnosis" class="form-control" rows="3"
                      placeholder="Enter diagnosis...">#encodeForHTML(prescription.diagnosis)#</textarea>
        </div>

        <div class="mb-4">
            <label class="form-label fw-bold">Notes</label>
            <textarea id="notes" class="form-control" rows="2"
                      placeholder="Optional notes...">#encodeForHTML(prescription.notes)#</textarea>
        </div>
                ...
                <tbody id="medicineBody">
                        ...
                        <tr>
                            <td>
                                <select class="form-select med-drug">
                                    ...
                                    <cfloop query="drugs">
                                        <option value="#drugs.drug_id#"
                                            #(drugs.drug_id EQ items.drug_id) ? 'selected' : ''#>
                                            #encodeForHTML(drugs.drug_name)#
                                            <cfif len(trim(drugs.strength))> (#encodeForHTML(drugs.strength)#)</cfif>
                                            <cfif len(trim(drugs.drug_form))> - #encodeForHTML(drugs.drug_form)#</cfif>
                                        </option>
                                    </cfloop>
                                </select>
                            </td>
                            <td><input type="text" class="form-control med-dosage"
                                       value="#encodeForHTML(items.dosage)#"
                                       placeholder="e.g. 500mg"></td>
                            <td><input type="text" class="form-control med-frequency"
                                       value="#encodeForHTML(items.frequency)#"
                                       placeholder="e.g. Twice daily"></td>
                            <td><input type="text" class="form-control med-duration"
                                       value="#encodeForHTML(items.duration)#"
                                       placeholder="e.g. 5 days"></td>
                            <td><input type="text" class="form-control med-instructions"
                                       value="#encodeForHTML(items.instructions)#"
                    ...
                    <cfelse>
                        <tr>
                            ...
                            <td><input type="text" class="form-control med-dosage" placeholder="e.g. 500mg"></td>
                            <td><input type="text" class="form-control med-frequency" placeholder="e.g. Twice daily"></td>
                            <td><input type="text" class="form-control med-duration" placeholder="e.g. 5 days"></td>
        ...
        <div class="d-flex gap-2">
            <button type="button" id="updatePrescriptionBtn" class="btn btn-success">
...
$(document).ready(function () {
    ...
    //  Update Prescription 
    $('#updatePrescriptionBtn').on('click', function () {
        var $btn      = $(this);
        ...
        $.ajax({
            ...
            if (getVal(res, 'success')) {
                ...
                $btn.prop('disabled', false)
                    .html('<i class="bi bi-check-circle me-1"></i> Update Prescription');
            }
        },
        error: function () {
            Swal.fire({
